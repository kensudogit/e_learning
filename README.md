...
| 領域 | 画面 | 主な API |
|------|------|----------|
| 申込・契約 | `/contracts` | `/api/v1/contracts`（個人/法人・チャネル・複数講座・クーポン・分割・変更キャンセル・見積請求領収・**紙教材の発送予定自動作成**） |
| 入金 | `/payments` | `/api/v1/payments`（入金記録・分割回次の消込） |
| アカウント | `/accounts` | `/api/v1/accounts/*` `/api/v1/organizations`（契約者/受講者・複数企業・家族メールリンク・ID変更・停止・PII削除） |
| 学習管理 | `/learning` · `/my` | `/api/v1/learning/*`（進捗・BM・理解度・督促・**学習履歴**） |
| 教材発送 | `/shipping` | `/api/v1/shipping/*`（発送時期・分割・在庫・住所・再発送・返送・海外・版管理・セット） |
| 添削 | `/assignments` | `/api/v1/assignments/*`（課題マスタ・提出・返却・**CorrectionResult/成績連携**） |
...
| 添削課題 | 提出・返却・ターンアラウンド・課題マスタ・成績/添削結果連携 |
| 試験・修了認定 | 試験提出、自動合否、修了証発行、成績・学習履歴記録 |
| 入金 | 入金記録・分割スケジュール消込（ゲートウェイ前の運用デモ） |
| 契約→発送 | 紙教材（`shipping_required`）の発送予定を契約作成時に自動登録 |
